  ...
  @objc open func drawDataSet(context: CGContext, dataSet: BarChartDataSetProtocol, index: Int)
  {
    ...
    for j in buffer.indices
    {
      ...
      if !isSingleColor
      {
        ...
        let gradientLocationInterval = 1.0 / Double(dataSet.colors.count)

        for index in 0...dataSet.colors.count - 1 {
          fillColors.append(dataSet.color(atIndex: index).cgColor)
          let location = locations[index] + gradientLocationInterval
