...
open class LineChartRendererWithLastCircle: LineRadarRenderer
{
  ...
  private func drawCircles(context: CGContext)
  {
    ...
    if let chart = dataProvider as? LineChartViewWithLastCircle {
  ...
  func drawGradientLine(context: CGContext, dataSet: LineChartDataSetProtocol, spline: CGPath, matrix: CGAffineTransform)
  {
    ...
    let gradientLocations: [CGFloat] = [0.11, 0.22, 0.33, 0.44, 0.56, 0.67, 0.78, 0.89, 1]

    //      gradientPositions.reversed()
    //        .map { (position) in
